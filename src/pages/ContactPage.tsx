...
// ... rest of the file stays same
          ...
          className="inline-flex items-center gap-2 px-4 py-1.5 bg-blue-500/10 text-blue-600 dark:text-blue-400 rounded-full text-xs font-black uppercase tracking-widest"
        >
        ...
        <motion.h1 
          ...
          className="text-4xl lg:text-6xl font-black tracking-tight text-slate-900 dark:text-white"
        >
        ...
        >
          Chúng tôi luôn sẵn sàng lắng nghe mọi ý kiến đóng góp và thắc mắc từ bạn. 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {contacts.map((contact, idx) => (
          ...
          >
            <div className={`w-16 h-16 rounded-2xl ${contact.color} text-white flex items-center justify-center mb-6 shadow-lg shadow-black/5 group-hover:scale-110 transition-transform`}>
              <contact.icon className="w-8 h-8" />
            </div>
            <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">{contact.name}</h3>
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-4">{contact.desc}</p>
            <div className="mt-auto pt-4 border-t border-slate-100 dark:border-white/5 w-full">
               <span className="text-xs font-black text-blue-600 dark:text-blue-400 group-hover:underline">{contact.value}</span>
            </div>
          </motion.a>
        ))}
      </div>
      ...
      <motion.div 
        ...
        className="relative bg-slate-900 dark:bg-blue-600 rounded-[3rem] p-8 lg:p-16 overflow-hidden"
      >
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12">
          <div className="space-y-6 text-center lg:text-left flex-1">
            <h2 className="text-3xl lg:text-5xl font-black text-white leading-tight">Gửi yêu cầu trực tiếp cho hệ thống</h2>
            <p className="text-lg text-slate-300 max-w-xl line-clamp-2">Bạn có ý tưởng hay hoặc cần hỗ trợ chuyên sâu? Đừng ngần ngại gửi tin nhắn trực tiếp.</p>
            <div className="flex flex-wrap items-center justify-center lg:justify-start gap-6">
              <div className="flex items-center gap-3 text-white">
                <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center border border-white/20">
                  <MapPin className="w-5 h-5" />
                </div>
                <span className="text-sm font-medium">Vietnam, Global</span>
              </div>
              <div className="flex items-center gap-3 text-white">
                <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center border border-white/20">
                  <Globe className="w-5 h-5" />
                </div>
                <span className="text-sm font-medium">Active Worldwide</span>
              </div>
            </div>
          </div>
          ...
          <button 
            ...
            className="shrink-0 bg-white text-slate-900 px-10 py-5 rounded-3xl font-black uppercase tracking-widest text-sm hover:scale-105 active:scale-95 transition-all shadow-2xl flex items-center gap-3"
          >
             <Send className="w-5 h-5 fill-slate-900" /> Nhắn ngay
          </button>
        </div>
      </motion.div>
    </div>
